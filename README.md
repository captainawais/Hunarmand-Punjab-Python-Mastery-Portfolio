# 🎓 Hunarmand Punjab Python Mastery Portfolio

![Python](https://img.shields.io/badge/Python-3.x-blue?style=for-the-badge&logo=python)
![Course](https://img.shields.io/badge/Hunarmand-Punjab-green?style=for-the-badge)
![Instructor](https://img.shields.io/badge/Instructor-Akbar_Ali-orange?style=for-the-badge)
![Status](https://img.shields.io/badge/Assignment-Completed-success?style=for-the-badge)
![Visitors](https://komarev.com/ghpvc/?username=captainawais&repo=Hunarmand-Punjab-Python-Mastery-Portfolio&label=Portfolio+Views&color=0e75b6&style=for-the-badge)


---

# 👨‍🎓 Student Information

**Name:** Awais Farooq

**Course:** Python Programming – Basics to Advanced

**Instructor:** Akbar Ali

**Program:** Hunarmand Punjab

**Year:** 2026

---

# 📖 About This Repository

This repository contains my complete practical work completed during the Hunarmand Punjab Python Programming course.

The objective of this portfolio is to demonstrate Python fundamentals, problem solving, object-oriented programming, file handling, exception handling, modules, and software development practices through practical implementation.

---

# 📚 Assignment Sections

- Part 01 – Python Basics
- Part 02 – Operators
- Part 03 – Control Statements
- Part 04 – Loops
- Part 05 – Functions
- Part 06 – Lists, Tuples & Sets
- Part 07 – Dictionaries
- Part 08 – File Handling
- Part 09 – Modules
- Part 10 – Exception Handling
- Part 11 – Object-Oriented Programming
- Final Integrated Project

---

# 🎯 Learning Objectives

✅ Python Fundamentals

✅ Problem Solving

✅ Data Structures

✅ Functions

✅ File Handling

✅ Exception Handling

✅ Object-Oriented Programming

✅ Real World Project Development

---

# 🚀 Final Integrated Project

## TECHFACT PRO – Student Course Management System

A complete Python-based Student Course Management System developed as the Final Integrated Project of the Hunarmand Punjab Python Programming Training Program.

### Key Features

* Student Management
* Course Management
* Enrollment Management
* Student Profiles
* File Handling Database
* Search Records
* Update Records
* Delete Records
* Object-Oriented Programming
* Exception Handling

### Technologies

* Python 3.x
* OOP
* File Handling
* Modules
* Functions
* Exception Handling

### Project Location

```text
Final-Integrated-Project/
```

### Documentation

Complete project documentation is available inside:

```text
Final-Integrated-Project/report/
```

### Version

Current Version: **1.0**

### Future Version

Version 2.0 will include:

* Course Analytics
* Dashboard Summary
* Search by Name
* Duplicate Validation
* Statistics Reports
* GUI Interface

### Author

Awais Farooq


---

⭐ Thank you for visiting this repository.